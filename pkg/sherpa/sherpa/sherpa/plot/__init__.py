...
__all__ = ('Plot', 'Contour', 'Point', 'SplitPlot', 'JointPlot',
           'DataPlot', 'DataContour', 'DelchiPlot',
...
def _interval_init(obj, fit, par):

    if obj.min is None or obj.max is None:
        ...
        v = (obj.max + obj.min) / 2.
        dv = numpy.fabs(v - obj.min)
        obj.min = v - obj.fac * dv
        obj.max = v + obj.fac * dv

    if( not numpy.isscalar( obj.min ) or
        not numpy.isscalar( obj.max ) ):
    ...
    if obj.delv is None:
        obj.x = numpy.linspace(obj.min,obj.max,obj.nloop)
    else:
        eps = numpy.finfo(numpy.float32).eps
        obj.x = numpy.arange(obj.min,obj.max+obj.delv-eps,obj.delv)

    x = obj.x
    if obj.log:
        if obj.max <= 0.0 or obj.min <= 0.0:
            raise ConfidenceErr('badarg', 'Log scale', 'on positive boundaries')
        obj.max = numpy.log10(obj.max)
        obj.min = numpy.log10(obj.min)

        x = numpy.linspace(obj.min,obj.max,len(x))
...
class IntervalProjection(Confidence1D):
    ...
    def prepare(self, fast=True, min=None, max=None, nloop=20,
                delv=None, fac=1, log=False):
        self.fast = fast
        Confidence1D.prepare(self, min, max, nloop, delv, fac, log)
    ...
    def calc(self, fit, par, methoddict=None):
        ...
        xvals = _interval_init( self, fit, par )

        par.freeze()
        
        def eval_proj(slice, fit, par, thawed):
            stats = numpy.array([])
            for val in slice:
                if self.log:
                    val = numpy.power(10, val)

#                 limits = _param_limit_update(par, val)

#                 try:
                par.val = val
                if len(thawed) > 1:
                    r = fit.fit()
                    stats = numpy.append(stats, r.statval)
                else:
                    stats = numpy.append(stats, fit.calc_stat())
#                 finally:
#                     _param_limit_rollback(par, *limits)

            return stats

        try:
            self.y = divide_run_parallel(eval_proj, xvals, fit, par, thawed)
        finally:
...
#                     _param_limit_rollback(par, *limits)
        ...
        try:
            ...
            self.y = divide_run_parallel(eval_uncert, xvals, fit, par)
...
# def _param_limit_update(par, val):
#     max = par.max
#     hmax = par._hard_max
#     if val > par.max:
#         par._hard_max = val
...
# def _param_limit_rollback(par, min, max, hmin, hmax):
#     par.max = max
#     par._hard_max = hmax
#     par.min = min
#     par._hard_min = hmin
...
def _region_init(obj, fit, par0, par1):

    if obj.levels is None:
        stat = fit.calc_stat()
        if obj.sigma is None or numpy.isscalar(obj.sigma):
            raise ConfidenceErr('needlist', 'sigma bounds')
        thelevels = numpy.zeros(len(obj.sigma), SherpaFloat)
        for i in range(len(obj.sigma)):
            thelevels[i] = stat - (2. * numpy.log(1. - erf(
                obj.sigma[i]/numpy.sqrt(2.))))
        obj.levels = thelevels
    ...
    for i in [0,1]:
        ...
        if obj.min[i] >= obj.max[i]:
            raise ConfidenceErr('badlimits')

        if numpy.isscalar( obj.nloop ) or obj.nloop[i] <= 1:
    ...
    if obj.delv is None:
        obj.x0 = numpy.linspace(obj.min[0],obj.max[0],obj.nloop[0])
        obj.x1 = numpy.linspace(obj.min[1],obj.max[1],obj.nloop[1])

    else:
        eps = numpy.finfo(numpy.float32).eps
        obj.x0 = numpy.arange(obj.min[0],obj.max[0]+obj.delv[0]-eps,obj.delv[0])
        obj.x1 = numpy.arange(obj.min[1],obj.max[1]+obj.delv[1]-eps,obj.delv[1])

    x = numpy.array([obj.x0, obj.x1])

    obj.x0, obj.x1 = numpy.meshgrid(obj.x0, obj.x1)
    obj.x0 = obj.x0.ravel()
    obj.x1 = obj.x1.ravel()

    for i in [0,1]:
        if obj.log[i]:
            if obj.max[i] <= 0.0 or obj.min[i] <= 0.0:
                raise ConfidenceErr('badarg', 'Log scale',
                                    'on positive boundaries')
            obj.max[i] = numpy.log10(obj.max[i])
            obj.min[i] = numpy.log10(obj.min[i])
            x[i] = numpy.linspace(obj.min[i],obj.max[i],len(x[i]))
...
class RegionProjection(Confidence2D):
    ...
    def calc(self, fit, par0, par1, methoddict=None):
        ...
        grid = _region_init( self, fit, par0, par1 )
        
        par0.freeze()
        par1.freeze()
        
        def eval_proj(slice, fit, par0, par1, thawed):
            stats = numpy.array([])
            for pars in slice:
...
#                 limits0 = _param_limit_update(par0, pars[0])
#                 limits1 = _param_limit_update(par1, pars[1])
...
#                 try:
                ...
                if len(thawed) > 2:
                    ...
                    stats = numpy.append(stats, fit.calc_stat())
#                 finally:
#                     _param_limit_rollback(par0, *limits0)
#                     _param_limit_rollback(par1, *limits1)

            return stats

        try:

            self.y = divide_run_parallel(eval_proj, grid, fit, par0, par1,
                                         thawed)
...
class RegionUncertainty(Confidence2D):
    ...
    def calc(self, fit, par0, par1, methoddict=None):
        ...
        grid = _region_init( self, fit, par0, par1 )
        stat = numpy.zeros(self.nloop[0]*self.nloop[1], SherpaFloat)

        for i in thawed:
            i.freeze()

        def eval_uncert(slice, fit, par0, par1):
            stats = numpy.array([])
            for pars in slice:
...
#                     _param_limit_rollback(par1, *limits1)
        ...
        try:
            ...
            self.y = divide_run_parallel(eval_uncert, grid, fit, par0, par1)
