...
from sherpa.data import Data2D, Data1D, BaseData
...
io_opt = str(io_opt).strip().lower() + '_backend'
...
def read_arrays(*args):
    ...
    return dstype('', *backend.get_column_data(*args))
...
def read_table(arg, ncols=2, colkeys=None, dstype=Data1D):
    ...
    # Used to raise errors for too many columns using DM syntax with Crates
    #nargs = get_num_args(dstype.__init__)[0]
    colnames, cols, name = backend.get_table_data(arg, ncols, colkeys)
...
def read_image(arg, coord='logical'):
    """
    read_image( filename [, coord='logical' ])

    read_image( IMAGECrate [, coord='logical' ])
    """
    data, filename = backend.get_image_data(arg)
    axlens = data['y'].shape

    #x1 = numpy.concatenate(list(i*numpy.ones((axlens[1],), SherpaFloat) + 1
    #                            for i in xrange(axlens[0])))
    #x0 = numpy.concatenate(list(repeat(numpy.arange(axlens[1],
    #                                                dtype=SherpaFloat) + 1,
    #                                   axlens[0])))
    ...
    data['coord'] = coord
    data['shape'] = axlens    
    return DataIMG(filename, x0.ravel(), x1.ravel(), **data)
